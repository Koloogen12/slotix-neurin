    ...
    <div className="flex min-h-screen flex-col lg:flex-row lg:gap-4 lg:p-4">
      ...
      <header className="glass-card sticky top-0 z-30 flex items-center gap-3 rounded-none rounded-b-3xl px-4 py-2.5 lg:hidden">
        ...
        <button
          ...
          className="ml-auto flex h-9 w-9 flex-none items-center justify-center rounded-full"
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 z-40 flex flex-col justify-end bg-black/20 lg:hidden"
          onClick={() => setMobileMenuOpen(false)}
        >
          <div
            className="glass-card m-2 flex flex-col gap-3 rounded-2xl p-4"
      ...
      <main className="min-w-0 flex-1 px-3 pb-24 pt-3 lg:px-3 lg:pb-10 lg:pt-0">{children}</main>

      {/* mobile bottom tab bar */}
      <nav className="glass-card fixed inset-x-2 bottom-2 z-30 flex rounded-2xl p-1 lg:hidden">
        {NAV_ITEMS.slice(0, 5).map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="flex flex-1 flex-col items-center gap-0.5 rounded-xl py-1.5 text-[10.5px] font-semibold"
              style={{ color: active ? "var(--color-link)" : "var(--color-muted)" }}
            >
              {item.icon}
              {item.label}
            </Link>
